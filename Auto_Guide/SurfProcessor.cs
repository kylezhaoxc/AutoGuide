...
{
    internal interface IMatchFilterInterface
    {
        int CountContours(Bitmap temp);
        double Getarea(PointF[] pts);
    }

    internal class SurfProcessor : IMatchFilterInterface
    {
        ...
        public double Getarea(PointF[] pts)
        {
            ...
            var pT = (top + right + middle)/2;
            var pB = (left + bottom + middle)/2;
            //边长明显过长，返回面积为0，即不会勾画单应矩阵
            if (top > 800 || right > 800 || left > 800 || right > 800) return 0;

            var obarea = Math.Sqrt(pT*(pT - top)*(pT - right)*(pT - middle)) +
                         Math.Sqrt(pB*(pB - left)*(pB - bottom)*(pB - middle));
            return obarea;
        }

        public Image<Bgr, byte> DrawResult(Image<Gray, byte> modelImage, Image<Gray, byte> observedImage,
            out long matchTime, out double area, int minarea, out Point center)
        {
            ...
            {
                watch.Stop();
                matchTime = watch.ElapsedMilliseconds; //dst = estimated_dist;
                return null;
            }
